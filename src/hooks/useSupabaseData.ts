...
export function useBundles() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("bundles")
        .select("*, bundle_items(*, products(*), brands(id, product_id, brand_name, price, tier, is_default_for_tier, size_variant, in_stock, stock_quantity, display_order, image_url, thumbnail_url, logo_url, compare_at_price))")
        .eq("is_active", true)
        .order("display_order");
      if (error) throw error;
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      let { data, error } = await supabase
        .from("bundles")
        .select("*, bundle_items(*, products(*, brands(id, product_id, brand_name, price, tier, is_default_for_tier, size_variant, in_stock, stock_quantity, display_order, image_url, thumbnail_url, logo_url, compare_at_price), product_sizes(*), product_colors(*)), brands(id, product_id, brand_name, price, tier, is_default_for_tier, size_variant, in_stock, stock_quantity, display_order, image_url, thumbnail_url, logo_url, compare_at_price))")
        .eq("slug", slug)
        .eq("is_active", true)
        .maybeSingle();
