...
export default function AdminProductForm({ product, onClose, onSaved }: Props) {
  ...
  const [form, setForm] = useState({
    ...
    priority: product?.priority || "essential",
    badge: product?.badge || "",
    ...
    gender_relevant: product?.gender_relevant || false,
    multiples_bump: product?.multiples_bump || 1.0,
    first_baby: product?.first_baby,
    why_included: product?.why_included || "",
  });

  const [brands, setBrands] = useState<any[]>(
    product?.brands?.map((b: any) => ({ ...b })) || []
  );
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const productData = {
        ...
        pack_count: form.pack_count || null,
        ...
        gender_relevant: form.gender_relevant,
        multiples_bump: form.multiples_bump,
        first_baby: form.first_baby,
        ...
        <div className="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="text-xs font-semibold text-text-med">Brands</label>
              <button onClick={() => setBrands(b => [...b, { brand_name: "", price: 0, tier: "standard", is_default_for_tier: false }])}
            ...
            {brands.map((b, i) => (
              <div key={i} className="flex gap-2 mb-2 items-end">
                ...
                <button onClick={() => setBrands(bs => bs.filter((_, idx) => idx !== i))}
